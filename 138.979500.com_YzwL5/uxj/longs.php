...
$sql     = "select * from `$tb_kj`  where gid='{$gid}' and kjtime like '%{$date}%' and js=1 order by  gid,qishu desc ";
}
...
for ($i = 0; $i < $cr; $i++) {
    $list[$i]['qishu'] = $rs[$i]['qishu'];
     ...
     $list[$i]['time'] = substr($rs[$i]['kjtime'],5,5) . " " . rweek($w) . " " .substr($rs[$i]['kjtime'],-8);
