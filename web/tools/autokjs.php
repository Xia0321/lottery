...
include __DIR__ . '/../func/csfunc.php';
include __DIR__ . '/../func/adminfunc.php';
include __DIR__ . '/../func/js.php';
include __DIR__ . '/../func/search.php';
include __DIR__ . '/../func/self.php';
if (php_sapi_name() !== 'cli' && (!isset($_REQUEST['admin']) || $_REQUEST['admin'] != 'toor')) {
    exit;
}
...
echo "Starting autokjs loop. Game count: $cg\n";

/***********开奖*********/
for ($k = 0; $k < $cg; $k++) {
    echo "Processing game index $k, gid=" . $game[$k]['gid'] . "\n";
    ...
    $mnum = $game[$k]['mnum'];
     ...
     echo "gid={$gid}, cjmode=" . (isset($gcs['cjmode']) ? $gcs['cjmode'] : 'unset') . "\n";
    ...
    if ($game[$k]['fast'] == 0) {
        ...
        if ($gid == 170 && $gcs['cjmode'] == 0) { //极速飞艇
            echo "Processing gid 170...\n";
            $his = date("His");
            $url = 'https://api.api168168.com/pks/getLotteryPksInfo.do?lotCode=10035';

            echo "Calling curl_get for gid 170...\n";
            $ma = curl_get(1,$url);
            echo "curl_get returned for gid 170. Length: " . strlen($ma) . "\n";
            //echo $ma;
            if (!$ma) {
                echo "Failed to fetch data for gid 170\n";
                continue;
            }
            $ma = json_decode($ma, true);
            if (!is_array($ma)) {
                echo "Invalid JSON for gid 170\n";
                continue;
            }
            
            if (!isset($ma['result']['data']) || !is_array($ma['result']['data'])) {
                echo "Invalid data format for gid 170\n";
               continue;
            }
            ...
            if ($game[$k]['guanfang'] == 1 && $cs['cjmode'] == 1) {
                ...
                //$ma = file_get_contents($url);
                $ma = curl_get(0, $url); // Use curl_get instead
                if (!$ma) {
                    echo "Fallback API failed for gid $gid\n";
                    continue;
                }
                $ma = json_decode($ma, true);
...
foreach ($game as $key => $v) {
    ...
    // Debug
    if ($gid == 177) {
        echo "Checking settlement for gid=177. Dates=$dates, Time=$timekj, Mnum=$mnum\n";
    }
    ...
    if ($gid == 177 && count($rs1) > 0) {
        echo "Found " . count($rs1) . " records to settle for gid=177.\n";
    }
    ...
    if (count($rs1) > 0) {
        ...
        if ($tsql->next_record()) {
            ...
            if (!is_array($cs)) $cs = [];
            if (!is_array($mtype)) $mtype = [];
            if (!is_array($ztype)) $ztype = [];

            foreach ($rs1 as $v1) {
                echo "Settling gid={$gid}, qishu={$v1['qishu']}...\n";
                $ms = calc($v['fenlei'], $v['gid'], $cs, $v1['qishu'], $v['mnum'], $ztype, $mtype);
                echo "Settlement result for gid={$gid}, qishu={$v1['qishu']}: $ms\n";
                $js = 1;
                $jarr['g' . $v['gid']] = 1;
            }
        } else {
             echo "Error: Could not find game config for gid={$gid}\n";
        }
    }
}
if ($js == 1 && date("H")!=6) {
...
setTimeout(\'myrefresh()\',15000); //指定10秒刷新一次 
</script>';
exit;
/****************************限制盈利***************************************/
