...
function myready(){
	...
	$(".news_tb").find("input.edit").click(function(i){
        ...
        $.ajax({type:'POST',url:mulu + 'news.php',data:'xtype=newsedit&id='+id+"&ifok="+ifok+"&agent="+agent+"&gundong="+gundong+"&alert="+aler+"&wid="+wid+"&cs="+cs+"&con="+con+"&time="+time,success:function(m){		
					if(Number(m)==1){
...
function delnews(idstr){
     $.ajax({type:'POST',url:mulu + 'news.php',data:'xtype=newsdel&id='+idstr,success:function(m){									  		 window.location.href=window.location.href;
	 }});
}