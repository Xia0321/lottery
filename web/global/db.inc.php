...
class lib_mysqli {  
    ...
    public function query($sql, $limit = null) {  
        ...
        if (!$this->rs) {  
            ...
            //echo "<p>sql: ".$sql."</p>";
            $error_msg = "DB Error: " . $this->mysqli->error . " | SQL: " . $sql;
            file_put_contents(__DIR__ . '/../uxj/debug_trace.txt', date('[Y-m-d H:i:s] ') . $error_msg . "\n", FILE_APPEND);
            die(json_encode(['err' => $error_msg]));
        } else {  
            $this->query_num++;  
            return $this->rs;  
        }  
	...
	public  function arr ($sql, $parameter)
	{
        ...
        if (!$query) {
            die("Invalid query: " . $sql . " Error: " . $this->mysqli->error);
        }
